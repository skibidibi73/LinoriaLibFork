...
-- 🌟 モバイル対応：マウスのターゲットまたは画面中央のターゲットを取得する関数
...
-- 🌟 カスタムUIの読み込み
local Library = loadstring(game:HttpGet('https://raw.githubusercontent.com/skibidibi73/LinoriaLibFork/refs/heads/main/in-dev'))()
...
-- 🌟 通知システムのフック処理 (HitLogなどにも連動)
local originalNotify = Library.Notify
function Library:Notify(Text, Time)
    -- Enable Notificationsがオフの場合はここで処理を止める（通知もHitlogも出ない）
...
Tabs.Settings.AddLeftGroupbox = function(self, name)
    if name == 'Themes' then
        ...
        -- 🌟 NEW: Backgroundタブにブラーとワイヤーフレームの設定を追加
...
local textUpdateDelay = 0.05 -- 変化速度
local pauseDuration = 1 -- 待機時間
...
local function createESP(player)
    ...
    weaponText.Font = Enum.Font.Code
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if method == "FireServer" and self.Name == "ShootEvent" then
        ...
        if type(bullets) == "table" then
            task.spawn(function()
                if Toggles.BulletTracers.Value and #bullets > 0 then
                    local lb = bullets[#bullets]
                    if lb then
                        local startPos = lb[1]
                        local char = LocalPlayer.Character
                        if char then
                        ...
                        pcall(DrawTracer_Optimized, startPos, lb[2])
                    end
                end
    ...
    elseif method == "InvokeServer" and self.Name == "FuncReload" then
        _G.RageShotsFired = 0
    end
...
RunService:BindToRenderStep("PhantomOverlay", Enum.RenderPriority.Last.Value + 200, function(deltaTime)
    ...
    if Toggles.CrosshairEnabled.Value then
        ...
        CH_TextLabel.Position = UDim2.fromOffset(basePos.X, basePos.Y + CH_Config.TextOffset)
