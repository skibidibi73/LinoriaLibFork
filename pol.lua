...
function Library:ApplyGlow(Inst)
    ...
    local baseStroke = Library:Create('UIStroke', {
    ...
    Library:AddToRegistry(baseStroke, { Color = 'AccentColor' })

    local layers = 15
    for i = 1, layers do
        local layerFrame = Library:Create('Frame', {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 1, 0),
            ZIndex = 0,
            Parent = GlowContainer
        ...
        local stroke = Library:Create('UIStroke', {
            Color = Library.AccentColor,
            Thickness = i * 2, 
            Transparency = 0.85 + (0.15 * (i / layers)), 
            ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
...
-- 🌟 修正版: Wireframe Dragging (元のUIを残したままドラッグし、離すとTPする + 枠線を細く)
                ...
                if Change.UserInputType == Enum.UserInputType.MouseMovement or Change == Input then
                    ...
                    if IsWindow and Library.WireframeDrag then
                        if not HasMoved and Delta.Magnitude > 2 then
                            ...
                            local stroke = Library:Create("UIStroke", {
                                Color = Library.AccentColor,
                                Thickness = 1, -- 🌟 ここを 2 から 1 に変更して線を細くしました
                                ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                                Parent = Wireframe
                            })
                        end
...
function Library:CreateWindow(...)
    ...
    -- 🌟 NEW: メインウィンドウにWireframeDragフラグを渡す
    ...
    MapNameLabel.TextColor3 = Color3.fromRGB(255, 50, 50) 
    ...
    function Library:Toggle()
        ...
        -- 🌟 NEW: UI開閉時の背景Blur連動アニメーション
        if Library.UseBlur then
            if Library.Toggled then
                Library.BlurEffect.Enabled = true
                TweenService:Create(Library.BlurEffect, TweenInfo.new(FadeTime, Enum.EasingStyle.Linear), {Size = Library.BlurSize}):Play()
            else
                local tween = TweenService:Create(Library.BlurEffect, TweenInfo.new(FadeTime, Enum.EasingStyle.Linear), {Size = 0})
                tween:Play()
